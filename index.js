...
function checkArray(obj) {
  ...
  return Array.isArray(obj);// Повертаємо результат перевірки
...
function combineAndReverseArrays(arr1, arr2) {
  ...
  const combiArr = arr1.concat(arr2);// Об'єднуємо два масиви за допомогою методу `concat`
  const reversArr = combiArr.reverse();// Обертаємо отриманий об'єднаний масив за допомогою методу `reverse`
  return reversArr;// Повертаємо об'єднаний та обернутий масив
}
...
function findElementIndexes(arr, element) {
 ...
 const firstIndex = arr.indexOf(element); // Знаходимо індекс першого знайденого елементу за допомогою методу `indexOf`
  ...
  const indexses =  [];// Створюємо пустий масив індексів знайдених елементів
  if (firstIndex !== -1) {
    indexses.push(firstIndex);
  }// Додаємо перший знайдений індекс, якщо елемент знайдено
  if (lastIndex !== -1 && lastIndex !== firstIndex) {
    indexses.push(lastIndex);
  }
  // Додаємо останній знайдений індекс, якщо він відрізняється від першого
   return indexses;// Повертаємо масив індексів
}
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  if (target < 0 || target >= arr.length || start < 0 || start >= arr.length || end < 0 || end > arr.length) {
    ...
    return arr; // Якщо неприпустимі індекси, повертаємо початковий масив без змін
  }

  // Копіюємо та переставляємо елементи за допомогою методу `copyWithin`
  const arrMod = arr.slice(); // Копіюємо початковий масив
  arrMod.copyWithin(target, start, end); // Використовуємо copyWithin для копіювання та переставлення елементів
  
  return arrMod; // Повертаємо модифікований масив
}
 ...
 */
 function customEvery(arr, condition) {
  if (!Array.isArray(arr) || typeof condition !== 'function') {
    return false; // Перевірка вхідних параметрів: якщо arr не масив або condition не є функцією, повертаємо false
  }

  return arr.every(condition); // Використання методу `every` для перевірки умови для кожного елементу масиву та повернення результату перевірки
}
...
function customUnshift(arr, ...elements) {
  Array.isArray(arr);// Перевіряємо, чи вхідний параметр є масивом
  const initialLength = arr.length;/// Зберігаємо початкову довжину масиву в змінну initialLength
    const newLength = arr.unshift(...elements); // Додаємо елементи на початок масиву за допомогою методу `unshift` та циклу for, початкове значення лічильника на 1 менше ніж довжина масива,
...
function customSome(arr, condition) {
  if (!Array.isArray(arr) || typeof condition !== "function") { return false}// Перевіряємо, чи вхідний параметр є масивом якщо ні повертаємо false
...
function customAt(arr, index) {
  ...
  if(typeof element === 'string' ) {console.log ('Елемент є рядком')};
  if(typeof element === 'number' ){console.log("Елемент є числом")};
  if(typeof element === 'object' ){console.log("Елемент є об'єктом")};
